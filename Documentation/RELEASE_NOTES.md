...
- Makes application capture guidance explicitly direct the user from the selected application to macOS's purple Share button. Source approval remains entirely inside `SCContentSharingPicker`; the signed 0.3.5 regression reproduction confirmed the system picker completes application selection.
...
- Adds deterministic selection, fit, time-mapping, persistence, and migration tests plus real AVFoundation reverse, crop, fade, audio-sample, and synchronized export coverage.
