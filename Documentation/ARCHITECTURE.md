...
AVCaptureSession ── camera CMSampleBuffer ► HEVC/H.264 “Camera” input
                           ...
                           TimelineProject + AVMutableComposition
                              ...
                              ├── microphone lane
                              ├── voiceover lane(s)
                              ├── mouse-follow zoom effect track
...
`CaptureCoordinator` owns user-visible state, persisted settings, privacy authorization, output naming, and the transition into the library. Display, application, and window approval use `SCContentSharingPicker` and remain available without a direct-capture grant. Picker-created application filters are display dependent: `CaptureStreamGeometry` converts the filter's screen-space content bounds into the owning display's logical coordinate space and applies that crop as `sourceRect`, so Monitor, the source recording, Library, Editor, mouse-follow zoom, and export all operate on the approved application extent instead of an offset full-display surface. Desktop-independent window filters retain ScreenCaptureKit's native full-window behavior because the framework ignores `sourceRect` for that filter style. Portion capture needs the one-time Direct Screen & System Audio Access approval that macOS describes as bypassing the private window picker; Reccy then presents one coordinated, non-capturable selection panel per connected display, and the accepted rectangle becomes the `sourceRect` of a display `SCContentFilter`.

`MultitrackRecorder` consumes ScreenCaptureKit sample buffers on one serial, user-interactive queue. When camera capture is enabled, `WebcamCaptureSession` uses native `AVCaptureSession` device discovery and `AVCaptureVideoDataOutput` for built-in, external, Continuity, and Desk View cameras. The session starts before writer configuration and resolves the camera track dimensions from the first delivered pixel buffer; samples begin flowing to the writer only after an input with that exact native aspect has been installed. This avoids stretching external cameras whose pre-start writer recommendation differs from the format the device actually negotiates. Camera timestamps are converted from the session synchronization clock to the host clock at the capture boundary, then enter the same writer queue and pause timeline as ScreenCaptureKit. The first complete screen frame establishes the asset-writer session time. Early camera and audio samples are buffered briefly and flushed from that same timestamp, preventing a secondary source from shifting sync.
...
Mouse-follow zoom is timeline metadata, never a destructive capture crop. Core Graphics provides the global pointer position without Accessibility or Input Monitoring access; `MouseFollowZoomSourceMapper` normalizes it into the approved display, portion, window, or application bounds. Capture smooths the path and stores at most 15 samples per second, with sparse stationary anchors, while Monitor transforms the existing screen preview and leaves its independent camera layer untouched. Starting or stopping the effect while paused uses the same pause-removed media clock as the writer. Finalization closes the effect at the writer's exact presentation duration and persists it in recording-manifest version 3, so live state and the saved timeline cannot drift at the recording boundary.
...
Project format 5 adds an optional mouse-follow zoom track. Each segment stores its timeline range, magnification, and normalized pointer path. Editor exposes that data as a first-class effect lane whose segments can be created, selected, resized, split with Split All, deleted, magnified, and adjusted by frame-accurate accessibility actions. Ripple deletion trims and shifts the effect with the media; extending a segment holds its nearest captured focus instead of inventing motion. Older projects and version-2 recording manifests decode with no effect track.

`TimelineCompositionBuilder` applies the effect only to the primary screen layer. It uses macOS 26's immutable `AVVideoCompositionLayerInstruction.Configuration` and native affine transform ramps across the captured pointer samples, including a short eased transition at each boundary. Pointer lookup is logarithmic, configuration size is bounded by the capture sampler, and the same video composition drives Editor, Library, and export. Camera instructions and caption overlays are separate downstream layers, so neither is magnified or displaced. Gap-fill transform coverage ensures an effect ending inside a held or black video gap returns to the base screen transform at the correct time.
...
Icon controls use one shared semantic-label and tooltip primitive so the visible interface, hover help, and assistive descriptions cannot drift. The timeline exposes each clip, effect, and gap as one adjustable accessibility object instead of leaking decorative handles. Frame-accurate move, trim, gap-fill, and mouse-zoom-boundary operations are available through VoiceOver custom actions and matching Editor-menu keyboard commands. The camera overlay is likewise one authoritative accessibility object: its position and size are spoken, and custom actions select, move, resize, or reset it without requiring a pointer drag.
...
The local 0.3.2 signed acceptance bundle covers every source/audio combination, native picker selection, a ten-minute sustained-load Apple-silicon run with hardware-encoder evidence and bounded A/V drift, an independently validated HDR10 capture, a ten-minute delivery export with one mixed audio program, independently validated Desk View and standard iPhone Continuity Camera recordings, and a complete spoken VoiceOver pass. A standard Continuity Camera fixture exposed severe camera-track end drift when the phone stopped producing frames; that run is preserved as regression evidence, the writer now aligns a material optional-camera tail or enters recovery, and focused plus full-suite checks pass. The exact rebuilt signed app then completed a 35.2-second post-fix standard Continuity recording with independent 2224-by-1440 screen and 640-by-480 camera video, system and microphone audio, and 80 milliseconds maximum track-end drift. Library and Editor playback retained the camera, and the rendered 2224-by-1440 HEVC delivery contained one mixed stereo AAC program with the camera visibly composited at 35.0 seconds. Other cross-environment acceptance requires multichannel/external microphone signal fixtures, visual HDR playback on separate confirmed SDR and HDR displays, and a signed Intel export.
