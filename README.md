...
Choose an entire display, every window from one application, or one specific window through Apple's private system picker. For application capture, select the application and then use the purple Share button supplied by macOS. Portion capture uses a direct QuickTime-style overlay across connected displays and a one-time approval that macOS describes as bypassing the private window picker. Reccy keeps the approved source obvious in the workspace and draws a local capture boundary that never appears in the recorded video.
...
- Drag the live camera overlay out of the way before or during a take; arrow-key and VoiceOver actions provide precise movement, and the normalized placement becomes that recording's non-destructive Editor and export default.
- Show or hide the pointer, highlight clicks, exclude Reccy's audio, and pause without leaving dead time in the result.
- Finish an active recording into the Library from the prominent Finish control, or use the quieter Cancel action and confirm permanent discard when a take should not be saved.
...
Monitor shows the exact incoming screen and camera pictures prepared for the writer during the countdown and throughout recording, plus elapsed time, committed file size, resolution, frame rate, recording state, separate rolling waveforms, and live source-aligned text for system audio and microphone input. Drag the camera overlay to reveal screen content beneath it, use arrow keys or VoiceOver actions for precise movement, or reset its placement from the native context menu. The normalized center is saved independently from camera size and supplies the recording's initial Editor, Library, and export layout without compositing the source track. Change or toggle mouse-follow zoom from one full-width native split control, review camera-effect state in a dismissible Reccy popover before opening Apple's authoritative Video Effects panel, or pause, resume, finish, and explicitly discard a take while this window remains open on another display.
...
- Keep the playhead in view automatically during playback without fighting manual navigation, selection, scrubbing, zooming, or the paused state; the ruler and track labels stay pinned to their native axes.
- Fit the complete project timeline at any window size with a duration- and viewport-derived minimum zoom, then zoom precisely back into clip detail.
- Select discontiguous clips with Command-click, extend an anchored range with Shift-click, add a range with Command-Shift-click, or use Command-A in the timeline. Move the selection as one rigid group and delete it through one non-destructive confirmation.
...
- Apply 0.25×, 0.5×, 1×, 2×, or 4× speed, forward/reverse playback, and independent fade-in/fade-out while linked picture and audio remain synchronized.
- Crop any video source and adjust its centered size from the native Video inspector; preview, saved projects, undo/redo, and export reuse the same normalized parameters.
...
The release-sized interaction audit and explicit deferrals are documented in [Documentation/EDITOR_ESSENTIALS.md](Documentation/EDITOR_ESSENTIALS.md).
...
<table>
  ...
  <tr>
    <td><strong>Menu bar</strong><br>Start a source-aware capture, monitor timer, safe-write status, and separate audio levels, toggle mouse-follow zoom, pause or finish, explicitly discard a take after confirmation, open Monitor, and return to recent recordings.</td>
...
| Timeline | Serializable recorded and imported media lanes, speed/reverse/fade/crop/size edits, poster frames, mouse-follow zoom effects, and editable timed captions materialized as `AVMutableComposition`, `AVVideoComposition`, and `AVAudioMix` |
...
The current suite exercises resolution policy, portrait and Retina sources, camera settings compatibility, persisted live-camera placement, camera-tail alignment after a material interruption, independent video-track composition, canonical preview track binding, overlay layout rendering, countdown/writer separation, confirmed live-recording discard, granular live mouse-zoom changes, poster-frame persistence, bounded still-image proxies, linked video/audio imports, Library and timeline multi-selection, rigid grouped movement, dynamic timeline fit, caption cue generation and timed burn-in export, forward/reverse and fast/slow time mapping, normalized crop/size rendering, video and audio fades, sample-accurate reverse audio, magnetic reorder, snapping, trimming, split and ripple operations, pause-time removal, track-specific waveforms, persistent per-gap fill choices, held-frame composition, manifests, bitrate-aware storage policy, cross-process recording leases, startup-failure cleanup, interrupted-file recovery, all ten export presets, audio-mix rendering, safe replacement, cancellation, transcript persistence/projection/correction/export, exact-track PCM extraction, and real post-recording plus live inference with every installed transcription engine. Model-backed tests skip cleanly on machines where the corresponding optional language asset or Whisper model is not installed. Installed-app Computer Use QA covers every workspace plus the real Library transport, source-aspect preview, saved caption and poster overlays, native editor toolbar, media import, caption editing, transcript search and seeking, accessible editor actions, menu-bar recording controls, live camera placement, live mouse-follow zoom, camera effects, and end-to-end export progress.
